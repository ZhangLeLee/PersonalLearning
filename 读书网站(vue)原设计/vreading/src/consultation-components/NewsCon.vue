<!-- 咨询 - 新闻 -->
<template>
  	<div id="newscon" class="row">
		<div class="col-8">
			<h4 class="zH4 textbody bgH4 py-3"><i class="fa fa-list-ul"></i>&nbsp;&nbsp;&nbsp;新闻</h4>
			<!-- 数据信息 -->
			<div class="row" v-for="myNew in news">
				...
				<div class="col-8">
					<div class="my-4">
						<router-link v-bind:to="'consultation/consultationInfo/'+myNew.conId" class="h5 newscon-h5 fontColor">{{myNew.newsTitle}}</router-link>
		...
		<div class="col-4">
			<consultation-side2></consultation-side2>
		</div>
  	</div> 
</template>

<script>
import ConsultationSide2 from './ConsultationSide2';
  	export default {
	    name:'newscon',
	    components:{
	    	ConsultationSide2
	    ...
	    created(){
			...
			this.$http.get('http://localhost:3000/consultation').then((res) => {
	       		for (var i = 0; i < res.body.length; i++) {
	       			if(res.body[i].conCategory == "news"){
